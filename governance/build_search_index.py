    ...
    for raw in open(path, encoding="utf-8"):
        ...
        if s.startswith("- path:"):
            ...
            cur = {"path": s.split("- path:", 1)[1].strip().strip('"'),
                   "enabled": True, "category": "", "origin": "internal", "note": ""}
        elif cur is not None:
            for key in ("enabled", "category", "origin", "note"):
                if s.startswith(key + ":"):
...
def collect_sources() -> list:
    ...
    # Family 2 — manifest-driven files, resolved against manifest_root.
    for e in parse_manifest(P["manifest"]):
        if not e["enabled"]:
            continue
        abs_path = os.path.join(P["manifest_root"], e["path"])
        if not os.path.exists(abs_path):
            print(f"  manifest entry missing on disk, skipped: {e['path']}")
            continue
        sources.append({"path": e["path"], "abs_path": abs_path,
                        "root": P["manifest_root"], "origin": e["origin"],
